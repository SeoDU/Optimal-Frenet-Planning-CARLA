...
class VehiclePIDController(object):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, args_lateral=None, args_longitudinal=None):
        ...
        """
        if not args_lateral:
            args_lateral = {'K_P': 3.0, 'K_D': 0.0, 'K_I': 0.0}
        if not args_longitudinal:
            args_longitudinal = {'K_P': 3.0, 'K_D': 0.0, 'K_I': 0.0}

        self._lon_controller = PIDLongitudinalController(**args_longitudinal)
        self._lat_controller = PIDLateralController(**args_lateral)
        self._last_control_time = rospy.get_time()

    def run_step(self, target_speed, current_speed, current_pose, waypoint, path_buf, N, vehicle_yaw):
        """
        ...
        """
        # target_speed is max value : 100
        diff_sum = 0
        if len(path_buf) > 1:
            for i in range(len(path_buf) - 1):
                diff_sum += abs(np.tan(path_buf[i + 1][2] - vehicle_yaw)) / (i+1) ** 2
            target_speed /= (1 + diff_sum)
            # print "log val",np.log(1 + 1.7 * (float(len(path_buf)) / N))
            target_speed *= np.log(1 + 1.7 * (float(len(path_buf) + 1) / N) **2)
            # print "log val", np.log(1 + 1.7 * (float(len(path_buf)+ 1) / N) **2)

        else:
            target_speed = 10

        # elif len(path_buf) == 1:
        #     print "path buf",path_buf
        #     target_speed = 10
        # elif len(path_buf) == 0:
        #     print "no path buf"
        #     target_speed = 30

        # print "N",N, "len path_buf",len(path_buf)
...
class PIDLongitudinalController(object):  # pylint: disable=too-few-public-methods
    ...
    def run_step(self, target_speed, current_speed, dt):
        ...
        # print "before clip", (self._K_P * _e) + (self._K_D * _de / dt) + (self._K_I * _ie * dt)
